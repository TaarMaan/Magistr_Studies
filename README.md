...
### 3.1 Автоматизация тестирования программного обеспечения
 ...
 Целью данного курса было получение всестороннего опыта реализаций автоматизированных тестов для покрытия всех частей современного веб-приложения. В ходе курса проведено курсовое проектирование с помощью сквозного тестирования на JavaScript с помощью cypress и cucumber в среде Visual Studio Code сайта ParaBank. Также, было разработано веб-приложение "Калькулятор", разделенное на 6 лабораторных работ для постепенного усложнения тестов и демонстрирования всех видов тестов, на всех этапах постройки приложения.
  
- 1 лабораторная представляет собой консольное приложение калькулятор на Java, использующее параметрические и динамические тесты функционала;
- 2 лабораторная представляет собой модернизированное приложение калькулятор, в котором тесты пишутся при помощи фреймворка testcontainers, данные записываются при помощи накатывания скриптов DDL и миграций в пустую бд, посредством фрейморвка для миграций из бд flywаy и системы управления реляционными базами данных PostgreSQL. Также, тесты поднимаются на другом сервере при помощи технологии контейнеризации из Docker;
...
### 3.2 Технологии промышленного программирования
  ...
  В данной дисциплине были рассмотрены способы запуска и организации взаимодействия параллельных процессов, нитей, работы с именованной памятью и службой реального времени. Работа представлена в виде отчетов по 3-м лабораторным работам, с приложенным кодом основных модулей в среде разработки QNX и части, отвечающей за виртуализацию всех процессов, посредством предоставления "сервера", в виде виртуальной машины Vmare.

### 3.3 Моделирование систем реального времени, распределенных и параллельных приложений на языке UML

В ходе данного курса были изучены способы создания микросервисной архитектуры Java приложений. Выполняя лабораторные работы, были получены навыки: 
- Навык работы с сервером авторизации - Keycloak;
- Навыки работы с программным обеспечением для автоматизации развертывания и управления приложениями в средах с поддержкой контейнеризации - Docker;
- Навыки проектирования и взаимодействия с базами данных - PostgreSQl;
- Навыки тестировани отклика и клиент-серверной связи между сервисами - Postman;
- Навыки реботы с брокером сообщений - Kafka.

В лабораторных работах было сделано Spring-приложение с микросервисной архитектурой на Java, состоящее из:
- В первой лабораторной работе
  1. Продукт для реализации single sign-on с возможностью управления доступом - keycloak, с созданным реалмом, клиентом (его токен мы помещаем в конфиг клауд сервера), двух пользователей с идентичными ролями (admin,user). Все это необходимо для single sign-on реализации аутентификации пользователя, админа и регистрации новых данных в базе данных в дальнейшем.
  2. Spring cloud config server, состоящих из настроек для подключения к серверу авторизации (токен первичного пользователя, база данных и.т.д.).
  3. База данных PostgreSQl включающая в себя информацию с сервера keycloak о пользовательской информации. Вся информация с сервера дублируется в базу, включая id.
  4. Spring-приложение, которое позволяет работать с пользователями в keycloak. Реализована через контроллер запросов с пользователей через Postman, репозитория с запросами в базу, сервисом по выполнению этой логики на локальном сервере. 
- Во второй лабораторной работе
  1. Добавились настройки сервера компани, лежащие на гитхабе.
  2. Добавились дополнительные эндпоинты по компаниям, также добавилась проверка существования компании в своем сервисе и все что касается пользователя обновилось добавлением компаний.
  3. Соответсвующую коллекцию в Postman обновить запросами на проверку компании и в запросы на информацию о юзерах, также добавилась информация про компании.
- В третьей лабороторной работе
  1. Добавлен новый сервис для Kafka, в связи с этим обновлена конфигурация сервера.
  2. Добавлен эндпоинт для удаления компании и выдача соответствующей ошибки 404, при поиски несуществуеюещей компании.
  3. Также пункт 2 добавляется в Postman.

### 3.4 Методы планирования Эксперимента и статической обработки информации

  В ходе курса по планированию эксперимента, были изучены методы статической обработки данных, в виде различных анализов, оценок и проведения необходимых экспериментов. Проведены 5 лабораторных работ, при помощи программы для работы с электронными таблицами Microsoft Excel и встроенного в нее расширения по работе со статическими данными.
- 1 лабораторная работа представляет собой дисперсионный анализ данных, включая в себя 5 видов распределения для случайно сгененированных чисел (Биномиальное, равномерное, Пуасонна, нормальное, Бернулли);
- 2 лабораторная работа состоит из генерации чисел по нормальному распределению и описательной статистики, состоящей из различных оценок выборок (Стандартная ошика, медиана, мода, коэф. Стьюдента, доверительная оценка и.т.д.);
- 3 лабортаорная, в ней проводится проверка однородности дисперсий, по средствам проверки дисперсийс помощью критерия Бартлетта по трем описательным статистикам, проведенным по трем одинаковым выборкам сгенерированнхы случайно при помощи равномерного распределения;
- 4 лабораторная, в данной работе были изучены методы планирования эскперимента и статистческой обработки информации по теме "Корреляционный и регресиооный анализ";
- 5 лабораторная на тему "Полный факторный эксперимент", была проведена в системе компьютерной алгебры Mathcad и включает в себя: 
   1. Воспроизведение планирования эксперимента и составлением матрицы плана.
   2. Программированем изначальныз выражения исследуемого объекта.
   3. Проведение, непосредственно, численного эксперимента и поиск неизвестных коэф. линейной модели.
   4. Проверка адекватности модели.
   5. Проверка усо=ловия значимости каждого из коэф. модели.
   6. построение графиков объектов и функции отклика.
